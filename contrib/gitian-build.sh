...
VERSION=
commit=false
url=https://github.com/soverainproject/soverain
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the soverain, gitian-builder, gitian.sigs, and soverain-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/soverainproject/soverain
...
then
	# Make output folder
	mkdir -p "./soverain-binaries/${VERSION}"
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit soverain=${COMMIT} --url soverain=${url} ../soverain/contrib/gitian-descriptors/gitian-linux.yml
	    ...
	    mv build/out/soverain-*.tar.gz build/out/src/soverain-*.tar.gz ../soverain-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit soverain=${COMMIT} --url soverain=${url} ../soverain/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/soverain-*.zip build/out/soverain-*.exe ../soverain-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit soverain=${COMMIT} --url soverain=${url} ../soverain/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/soverain-*.tar.gz build/out/soverain-*.dmg ../soverain-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit soverain=${COMMIT} --url soverain=${url} ../soverain/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../soverain/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/soverain-*.tar.gz build/out/src/soverain-*.tar.gz ../soverain-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../soverain/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/soverain-*win64-setup.exe ../soverain-binaries/${VERSION}
	    mv build/out/soverain-*win32-setup.exe ../soverain-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../soverain/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/soverain-osx-signed.dmg ../soverain-binaries/${VERSION}/soverain-${VERSION}-osx.dmg
	fi
	popd || exit
