...
#ifndef SOVERAIN_ZSOVEWALLET_H
#define SOVERAIN_ZSOVEWALLET_H
...
public:
    CzSOVEWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZSOVE(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZSOVE(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //SOVERAIN_ZSOVEWALLET_H
