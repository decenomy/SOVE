...
CzSOVEWallet::CzSOVEWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zsove seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSOVESeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSOVESeed_deprecated()) {
                    LogPrintf("%s: Updated zSOVE seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zsove seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zsove wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSOVEWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSOVECount(nCountLastUsed);
    else if (!walletdb.ReadZSOVECount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSOVEWallet::Lock()
{
    seedMaster = 0;
}

void CzSOVEWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSOVEWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSOVE(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSOVEWallet::LoadMintPoolFromDB()
{
...
void CzSOVEWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSOVEWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSOVEWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zsoveTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSOVEWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSOVE(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zsoveTracker which also adds to database
    pwalletMain->zsoveTracker->Add(dMint, true);
...
void CzSOVEWallet::SeedToZSOVE(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSOVEWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSOVEWallet::GenerateDeterministicZSOVE(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSOVEWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSOVEWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
