...
bool CMasternodeConfig::read(std::string& strErr)
{
    ...
    if (!streamConfig.good()) {
        ...
        if (configFile != NULL) {
            std::string strHeader = "# Masternode config file\n"
                                    ...
                                    "# Example: mn1 127.0.0.2:18976 88Pb77CdDNm2WmqDn8YpYV2y6RQX7hsX11U7fxWk1U28EoHMZb3 79b352ef6aa3a1994a4c3d711e2d460385b53cce065611049a9a3c55e49b53e1 0\n";
    ...
    for (std::string line; std::getline(streamConfig, line); linenumber++) {
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (CService(ip).GetPort() != 18976) {
                strErr = _("Invalid port detected in masternode.conf") + "\n" +
                         strprintf(_("Line: %d"), linenumber) + "\n\"" + line + "\"" + "\n" +
                         _("(must be 18976 for mainnet)");
                streamConfig.close();
                return false;
            }
        } else if (CService(ip).GetPort() == 18976) {
            strErr = _("Invalid port detected in masternode.conf") + "\n" +
                     strprintf(_("Line: %d"), linenumber) + "\n\"" + line + "\"" + "\n" +
                     _("(18976 could be used only on mainnet)");
            streamConfig.close();
            return false;
        }
