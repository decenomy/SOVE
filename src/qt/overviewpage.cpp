...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::SOVE)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sSOVEPercentage, QString& szSOVEPercentage)
{
    int nPrecision = 2;
    ...
    szSOVEPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sSOVEPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount soveAvailableBalance = balance - immatureBalance - nLockedBalance;
    ...
    // Combined balances
    CAmount availableTotalBalance = soveAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // SOVE labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnitComma(nDisplayUnit, soveAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // Percentage labels
    ui->labelSOVEPercent->setText(sPercentage);
    ui->labelzSOVEPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zSOVE.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in soverain.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in soverain.conf");
    }
    ...
    bool showSOVEAvailable = settingShowAllBalances || soveAvailableBalance != nTotalBalance;
    bool showWatchOnlySOVEAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showSOVEPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlySOVEPending = watchUnconfBalance != 0;
    bool showSOVELocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlySOVELocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showSOVEAvailable || showWatchOnlySOVEAvailable);
    ui->labelBalanceText->setVisible(showSOVEAvailable || showWatchOnlySOVEAvailable);
    ui->labelWatchAvailable->setVisible(showSOVEAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showSOVEPending || showWatchOnlySOVEPending);
    ui->labelPendingText->setVisible(showSOVEPending || showWatchOnlySOVEPending);
    ui->labelWatchPending->setVisible(showSOVEPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showSOVELocked || showWatchOnlySOVELocked);
    ui->labelLockedBalanceText->setVisible(showSOVELocked || showWatchOnlySOVELocked);
    ui->labelWatchLocked->setVisible(showSOVELocked && showWatchOnly);
    ...
    bool showzSOVEAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzSOVEUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzSOVEImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzSOVEAvailable);
    ui->labelzBalanceMatureText->setVisible(showzSOVEAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzSOVEUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzSOVEUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzSOVEImmature);
    ui->labelzBalanceImmatureText->setVisible(showzSOVEImmature);
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelSOVEPercent->setVisible(showPercentages);
    ui->labelzSOVEPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("SOVE")
    updateDisplayUnit();
}
