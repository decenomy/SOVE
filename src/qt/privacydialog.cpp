                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->zSOVEpayAmount->setAttribute(Qt::WA_MacShowFocusRect, 0);
    

    // "Spending 999999 zSOVE ought to be enough for anybody." - Bill Gates, 2017
    ui->zSOVEpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzSOVESyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zSOVEpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzSOVE_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSOVE is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zSOVE...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zSOVE in ") +
...
void PrivacyDialog::on_pushButtonSpendzSOVE_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSOVE is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zSOVE
        sendzSOVE();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zSOVE
    sendzSOVE();
}
...
void PrivacyDialog::sendzSOVE()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Soverain Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zSOVEpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zSOVE is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " SOVE </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zSOVE</b>";
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zSOVE require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zSOVE"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZSOVE_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zSOVE require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zSOVE"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zSOVE was spent successfully update the addressbook with the label
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zSOVE Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zsoveTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zSOVE </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zSOVE "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zSOVE "));
    ui->labelzSOVEAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzSOVESyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzSOVE->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zSOVE
        ui->pushButtonMintzSOVE->setEnabled(false);
        ui->pushButtonMintzSOVE->setToolTip(tr("zSOVE is currently disabled due to maintenance."));

        // Spend zSOVE
        ui->pushButtonSpendzSOVE->setEnabled(false);
        ui->pushButtonSpendzSOVE->setToolTip(tr("zSOVE is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zSOVE
        ui->pushButtonMintzSOVE->setEnabled(true);
        ui->pushButtonMintzSOVE->setToolTip(tr("PrivacyDialog", "Enter an amount of SOVE to convert to zSOVE", 0));

        // Spend zSOVE
        ui->pushButtonSpendzSOVE->setEnabled(true);
        ui->pushButtonSpendzSOVE->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}