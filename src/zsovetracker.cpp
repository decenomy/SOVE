...
CzSOVETracker::CzSOVETracker(std::string strWalletFile)
{
...
CzSOVETracker::~CzSOVETracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSOVETracker::Init()
{
...
bool CzSOVETracker::Archive(CMintMeta& meta)
{
...
bool CzSOVETracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSOVETracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSOVETracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSOVETracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzSOVETracker::GetSerialHashes()
{
...
CAmount CzSOVETracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSOVETracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSOVETracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSOVETracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSOVETracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSOVETracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSOVETracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSOVETracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSOVETracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSOVETracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSOVETracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSOVETracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSOVETracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSOVETracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSOVETracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSOVETracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSOVETracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzsove from DB\n", __func__, listDeterministicDB.size());
    }
