...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zsoveTracker = unique_ptr<CzSOVETracker>(new CzSOVETracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZSOVE_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZSOVE_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
